...
## v4.3.1 图片服务接口修复（2026-09-04）

- 图片服务默认地址切换为 `https://cf.api.fan`；服务端下发旧官方地址时，客户端自动迁移到新地址，文生图与图生图接口路径保持不变。
- 图片任务、图片测试及网络诊断共用地址解析，其他自定义服务地址和文本／视觉 BYOK 配置保持不变。
- 历史记录优先显示实际错误中的接口地址；无地址记录时仅显示接口路径，避免旧任务被错误标记为当前服务地址。
- 服务端图片运行时配置同步切换到新地址。
- 发布前检查：TypeScript 检查、前端生产构建通过；前端测试 2570 passed / 10 skipped；Rust 测试 236 passed / 1 ignored。真实生产生图与图生图验收单独记录，不以单元测试替代。
